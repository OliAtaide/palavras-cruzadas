  ...
  $.crossword = function (data, options) {
    ...
    var impl = {
      ...
      appendGrid: function (options) {
        ...
        var gridSize = {
          width: gridWidth ? gridWidth : 0,
          height: gridHeight ? gridHeight : 0
        };
        ...
        var grid = $(
          '<table id="cwd-grid" cellSpacing="0" rowSpacing="0"></table>'
        );
        var clueNo = 1;
        ...
        var newTile = function (row, col) {
          return (
            $(
              impl.stringFormat(
                '<td class="cwd-tile" row="{0}" col="{1}" />',
                row,
                col
              )
            )
              // Must add at least one space to get borders showing in IE7.
              .append($('<div class="cwd-tile-letter" />').text(" "))
          );
        };
        ...
        for (i = 0; i < gridSize.height; i++) {
          ...
          for (j = 0; j < gridSize.width; j++) {
            ...
            if (
              typeof options.gridMask[i] !== "undefined" &&
              options.gridMask[i].charAt(j) === " "
            ) {
              tile.addClass("cwd-tile-active").css("background-color", "White");
            } else {
              tile
                .addClass("cwd-tile-inactive")
        ...
        grid.find("td.cwd-tile-active").each(function () {
          ...
          var acrossNo =
            (j === 0 || options.gridMask[i].charAt(j - 1) !== " ") &&
            j < gridSize.width - 1 && options.gridMask[i].charAt(j + 1) === " ";

          // Inactive tile above (or edge) AND active tile below => start of down clue.
          var downNo =
            (i === 0 || options.gridMask[i - 1].charAt(j) !== " ") &&
            i < gridSize.height - 1 &&
            options.gridMask[i + 1].charAt(j) === " ";

          var row = i,
            col = j;
          if (acrossNo) {
            while (col < gridSize.width) {
              var thisTile = grid.find(
                impl.stringFormat("td[row={0}][col={1}]", row, col)
              );
            ...
            clues.push(
              impl.buildClueObject(
                options.acrossClues[acrossClueIndex],
                index++,
                clueNo,
                col - j,
                "a",
          ...
          if (downNo) {
            ...
            clues.push(
              impl.buildClueObject(
                options.downClues[downClueIndex],
                index++,
                clueNo,
                row - i,
                "d",
          ...
          $.each(value.multiParts, function (index2, subClueText) {
            ...
            if (subClue) {
              ...
              subClue.text = impl.stringFormat(
                "See {0}{1}",
                rootClue.number,
                rootClue.bothWays ? rootClue.direction : ""
              );
        ...
        // Attach a clue box?
        var clueBox =
          options.clueBox === true
            ? $('<div id="cwd-clue-box" />').append(
              '<span id="cwd-clue-box-text" />'
            )
            : "";
      ...
      buildClueObject: function (
        ...
        if (!cluePattern && clueAnswer) {
          ...
          for (i = 0; i < clueAnswer.length; i++) {
            if (/[A-Z][0-9]/i.test(clueAnswer.charAt(i))) {
              partLength++;
            } else {
              ...
              cluePattern = cluePattern.concat(
                clueAnswer.charAt(i) === "-" ? "-" : ","
              );
              partLength = 0;
              // Skip over any further non-alpha characters.
              while (
                !/[A-Z][0-9]/i.test(clueAnswer.charAt(i + 1)) &&
                i + 1 < clueAnswer.length
              ) {
                i++;
              }
            }
      ...
      bindEvents: function () {
        ...
        $("#cwd-grid td.cwd-tile-active").click(function () {
          ...
          if ($(this).hasClass("cwd-tile-highlight")) {
            prevDirection =
              $(this)
                .prev("td.cwd-tile-active")
                .hasClass("cwd-tile-highlight") ||
                $(this)
                  .next("td.cwd-tile-active")
                  .hasClass("cwd-tile-highlight")
                ? "a"
                : "d";
          }
        ...
        $(document).keydown(function (event) {
          ...
          if (highlightedTiles.length > 0) {
            ...
            var direction =
              highlightedTiles.filter("[cursorIndex=0]").attr("row") ===
                highlightedTiles.filter("[cursorIndex=1]").attr("row")
                ? "a"
                : "d";
            ...
            if (code === 8) {
              if (
                ...
                (cursorIndex > 0 &&
                  highlightedTiles
                    .filter(
                      impl.stringFormat("[cursorIndex={0}]", cursorIndex - 1)
                    )
                ...
                $(".cwd-tile-correct,.cwd-tile-incorrect")
                  .removeClass("cwd-tile-correct")
                  .removeClass("cwd-tile-incorrect");
              }
            } else {
              ...
              if (/[A-Z0-9]/i.test(charEntered)) {
                ...
                $(
                  ".cwd-tile-letter",
                  highlightedTiles.filter(
                    impl.stringFormat("[cursorIndex={0}]", cursorIndex)
                  )
                )
                  .text(charEntered)
                  .attr(direction, "true");
                ...
                // while (
                //   cursorIndex < highlightedTiles.length &&
                //   highlightedTiles
                //     .filter(impl.stringFormat("[cursorIndex={0}]", cursorIndex))
                //     .find("div.cwd-tile-letter")
                //     .text() !== " "
                ...
                if (cursorIndex < highlightedTiles.length) {
                  ...
                  if (
                    options.validateAnswer === "clue" ||
                    options.validateAnswer === "grid"
                  ) {
                    var enteredAnswer = "",
                      actualAnswer = "";
                        ...
                        if (impl.isGridCompleted()) {
                          $.each(clues, function (index, value) {
                            if (clue.answer && clue.enteredAnswer) {
                              if (
                                clue.answer.toUpperCase() !== clue.enteredAnswer
                              ) {
                                correct = false;
                              }
                            } else {
      ...
      appendClues: function (data, onRight) {
        ...
        var cluesDiv = $(
          impl.stringFormat(
            '<div id="cwd-clues" style="vertical-align: top; margin-left:20px; display: {0}" />',
            displayStyle
          )
        );
      ...
      showClue: function (clueId) {
        ...
        $("#cwd-clues span.cwd-clue-highlight").removeClass(
          "cwd-clue-highlight"
        );
        ...
        if (clueId >= 0) {
          ...
          $.each(clueIds, function (index, value) {
            ...
            var clue = $(
              impl.stringFormat("#cwd-clues #cwd-clueText-{0}", value)
            ).addClass("cwd-clue-highlight");
      ...
      columniseClues: function (onRight) {
        ...
        var getNewColumn = function (number) {
          return $(
            impl.stringFormat(
              '<div id="cwd-col{0}" style="position: relative; display: inline-table; vertical-align: top; width: {2};" />',
              number,
              columnWidth
            )
          );
      ...
      stringFormat: function () {
        ...
        if (typeof resultString != "undefined") {
          resultString = resultString.replace(/{(\d+)}/g, function (
            match,
            number
          ) {
